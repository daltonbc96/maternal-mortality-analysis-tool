createAnualPrimeiroBarPlot <- function(processedDataNacional, processedDataLevel1, intervaloAnos, 
    ...
    plot <- ggplot(fullData, aes(x = reorder(location, location, FUN = function(x) -length(x)), 
                                 y = count, fill = as.factor(year), text = hover_text)) +
